...
// Convert 'YYYY-M' format to 'YYYY-MM-01' and 'YYYY-MM-t' for SQL query
...
FROM pos_orders
WHERE DATE(orders_date) BETWEEN ? AND ?
...
if ($stmt = $conn->prepare($sql_query)) {
    ...
    if ($result->num_rows > 0) {
        ...
        $sheet->setCellValue('A1', 'Monthly Sales Report - Walkin');
        ...
        while ($row_data = $result->fetch_assoc()) {
            $monthName = $monthNames[str_pad($row_data['order_month'], 2, '0', STR_PAD_LEFT)];
            $sheet->setCellValue('A' . $row, $monthName . ' ' . $row_data['order_year']);
        ...
        header('Content-Disposition: attachment;filename="monthly_sales_walkin_' . $todaydate . '.xlsx"');
