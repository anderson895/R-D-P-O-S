function fetStoreTable() {
    $.ajax({
        url: '../../POS/functions/get_report_monthly_online.php',
        type: 'GET',
        dataType: 'json',
        success: function(response) {
            ...
            $('#search_monthly').on('input', function() {
            ...
            $('#itemsPerPage_monthly').on('change', function() {
            ...
            function renderTable() {
                const tableBody = $('#tableBody_monthly');
                const searchText = $('#search_monthly').val().toLowerCase();
                ...
                $('#info_monthly').text(`Showing ${Math.min(rowsPerPage, totalItems)} of ${totalItems} items.`);
            }

            function setupPagination() {
                const pagination = $('#pagination_monthly');
                const searchText = $('#search_monthly').val().toLowerCase();
                const filteredData = data.filter(item => {
                    const monthName = monthNames[parseInt(item.order_month) - 1].toLowerCase();
                    const year = item.order_year.toLowerCase();
            ...
            $('#tableHead_monthly th').on('click', function() {
