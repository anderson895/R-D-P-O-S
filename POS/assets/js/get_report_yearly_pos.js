function fetStoreTable() {
    $.ajax({
        url: '../../POS/functions/get_report_yearly_pos.php',
        type: 'GET',
        dataType: 'json',
        success: function(response) {
            ...
            $('#search_yearly_pos').on('input', function() {
                currentPage = 1;
                renderTable();
                setupPagination();
            });

            $('#itemsPerPage_yearly_pos').on('change', function() {
            ...
            function renderTable() {
                const tableBody = $('#tableBody_yearly_pos');
                const searchText = $('#search_yearly_pos').val().toLowerCase();
                ...
                $('#info_yearly_pos').text(`Showing ${Math.min(rowsPerPage, totalItems)} of ${totalItems} items.`);
            }

            function setupPagination() {
                const pagination = $('#pagination_yearly_pos');
                const searchText = $('#search_yearly_pos').val().toLowerCase();
                const filteredData = data.filter(item => {
                    const year = item.order_year.toLowerCase();
                    const transactionCode = `SLS${String(item.id).padStart(4, '0')}`.toLowerCase();
            ...
            $('#tableHead_yearly_pos th').on('click', function() {
