...
?>
            <select class="form-control" id="selectRider" data-id="<?= $order['order_id'] ?>">
                ...
                ?>
                    <option readonly <?php if($assignRider==$rider['acc_id']){ echo "selected";} ?> value="<?= $rider['acc_id'] ?>" <?= ($rider['acc_id'] == $order['rider_id']) ? 'selected' : '' ?>><?= $rider['acc_fname'] . ' ' . $rider['acc_lname'] ?></option>
                <?php
                }
                ?>
            </select>
