...
if ($connections->query($sql) === TRUE) {
    ...
    VALUES('$acc_id', 'Update e-wallet name from`$db_payment_name` changed to `$updated_db_payment_name`', '$currentDateTime','product','$payment_id')");
    }    

    if($db_payment_number!=$db_payment_number){
    mysqli_query($connections, "INSERT INTO users_log(act_account_id, act_activity, act_date, act_table, act_collumn_id)
    VALUES('$acc_id', 'Update e-wallet number from`$db_payment_number` changed to `$updated_db_payment_number`', '$currentDateTime','product','$payment_id')"); 
     }   

    if($db_payment_image!=$updated_db_payment_image){
        mysqli_query($connections, "INSERT INTO users_log(act_account_id, act_activity, act_date, act_table, act_collumn_id)
        VALUES('$acc_id', 'Update $updated_db_payment_name` e-wallet image', '$currentDateTime','product','$payment_id')");     
    } 
    if($db_payment_status!=$updated_db_payment_status){
        if($updated_db_payment_status=="0"){
            $updated_db_payment_status_log=="Disabled E-wallet $updated_db_payment_name";
        }else{
            $updated_db_payment_status_log=="Enabled E-wallet $updated_db_payment_name";
        }
        mysqli_query($connections, "INSERT INTO users_log(act_account_id, act_activity, act_date, act_table, act_collumn_id)
