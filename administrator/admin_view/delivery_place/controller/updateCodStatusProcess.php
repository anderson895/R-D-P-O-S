...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "UPDATE tbl_address SET address_cod = ?, address_date_edited = ? WHERE address_id = ?";

    // Use a prepared statement to update the category
    if ($stmt = $connections->prepare($sql)) {
        $stmt->bind_param("ssi", $status_cod, $currentDateTime, $address_id);
        ...
        if ($stmt->execute()) {
            ...
            $address_complete_name = $row["address_complete_name"];
                ...
                VALUES ('$acc_id', 'Set `$address_complete_name` $status_log cash on delivery(COD)', '$currentDateTime','shipping',$address_id)");
