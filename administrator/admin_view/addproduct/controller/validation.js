$(document).ready(function () {
  ...
  //     numericValue = Math.min(parseInt(numericValue, 10), 999);
  //     $(this).val(numericValue);
  // });

  $kg.on("input", function () {
    var inputValue = parseFloat($(this).val());
    if (inputValue < parseFloat($(this).attr("min"))) {
      $(this).val($(this).attr("min"));
    }
    if (inputValue > parseFloat($(this).attr("max"))) {
      $(this).val($(this).attr("max"));
    }
  });

  var $ml = $("#ml");
  $ml.on("input", function () {
    var inputValue = parseFloat($(this).val());
    if (inputValue < parseFloat($(this).attr("min"))) {
      $(this).val($(this).attr("min"));
    }
  ...
  $productName.on("input", function () {
    ...
    // Bantayan ang haba ng teksto at i-trim kung sobra sa limit
  ...
  function validateForm() {
    ...
    if (imageFileName === "") {
      allFieldsValid = false;
      $errorProductImage.text("Please upload a product image.");
