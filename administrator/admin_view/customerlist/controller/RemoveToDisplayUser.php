...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $acc_display_status = $_POST['acc_display_status'];


    // Gumawa ng SQL query para sa pag-update ng prod_status
    $sql = "UPDATE account SET acc_display_status = ? WHERE acc_id = ?";
    mysqli_query($connections,"UPDATE account SET acc_status ='2' WHERE acc_id = $acc_id ");
    ...
    if ($stmt = $connections->prepare($sql)) {
        $stmt->bind_param("ii", $acc_display_status, $acc_id);
